...
class TargetEdgeInitializer(nn.Module):
    ...
    def forward(self, data):
        ...
        # Only consider k-hop neighbors instead of all possible pairs
        k_hop_adj = torch.matmul(edge_index, edge_index.T)  # for 2-hop neighbors
        xt = torch.matmul(k_hop_adj, x)
...
class STPGSR(nn.Module):
    ...
    def forward(self, source_pyg, target_mat):
        ...
        target_edge_init = self.target_edge_initializer1(source_pyg)
